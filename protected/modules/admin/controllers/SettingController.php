...
//                }
    ...
    {
        $this->layout ='column1';
        $model = new SettingPatientForm;
        $model->scenario = "updateSettings";
        $setting = Yii::app()->setting;
        $listAttributes = SettingPatientForm::getAllAttributes();
        if (isset($_POST['SettingPatientForm'])) {
            $model->attributes = $_POST['SettingPatientForm'];
            if ($model->validate()) {
                if ($listAttributes && array($listAttributes))
                {
                    foreach($listAttributes as $attr)
