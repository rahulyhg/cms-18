...
class ProductsimgController extends AdminController
{
	...
	public function actionCreate()
	{
		try
		{
			$model = new Product_img('create');
			if (isset($_POST['Product_img']))
			{
				$model->attributes = $_POST['Product_img'];
	...
	public function actionIndex()
	{
		try
		{
            $modelAdd = new Product_img('create');
            if (isset($_POST['Product_img']))
            {
               // var_dump($modelAdd);
                $modelAdd->attributes = $_POST['Product_img'];
                $modelAdd->product_id = $_GET['id'];
                if ($modelAdd->save())
                {
                    $modelAdd->saveImage('product_image');
                    $this->setNotifyMessage(NotificationType::Success, $this->singleTitle . ' has been created');
                   // $this->redirect(array('view', 'id' => $modelAdd->id));
                }
                else
                    $this->setNotifyMessage(NotificationType::Error, $this->singleTitle . ' cannot be created for some reasons');
            }

			$model = new Product_img('search');
			$model->unsetAttributes();  // clear any default values
			$model->product_id=$_GET['id'];  // clear any default values
	...
	{
		$deleteItems = $_POST['product_img-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDetele);
		if (!empty($shouldDelete))
		{
			$deleteImages = Product_img::model()->findAll('id in (' . implode(',', $shouldDelete) . ')');
			...
			} Product_img::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
