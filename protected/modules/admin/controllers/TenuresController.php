...
class TenuresController extends AdminController
{
    public $pluralTitle = 'Tenures';
    public $singleTitle = 'Tenures';
    public $cannotDetele = array();
    public function actionCreate(){
        try {
            $model = new Tenures('create');
             $model->display_order = $model->nextOrderNumber();
            if (isset($_POST['Tenures'])) {
                $model->attributes = $_POST['Tenures'];
                if($model->save())
				{
    ...
    public function actionIndex() {
        try {
            $model=new Tenures('search');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['Tenures']))
                $model->attributes=$_GET['Tenures'];
	...
	*/
	...
	{
		$deleteItems = $_POST['tenures-grid_c0'];
		$shouldDelete = array_diff($deleteItems, $this->cannotDetele);

		if (!empty($shouldDelete))
		{
						Tenures::model()->deleteAll('id in (' . implode(',', $shouldDelete) . ')');
