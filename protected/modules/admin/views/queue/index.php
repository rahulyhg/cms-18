...
$('#clearsearch').click(function(){
	...
	$.fn.yiiGridView.update('queue-grid', {data: data});
	return false;
});

$('.deleteall-button').click(function(){
        var atLeastOneIsChecked = $('input[name=\"queue-grid_c0[]\"]:checked').length > 0;
        ...
        {
                document.getElementById('queue-grid-bulk').action='" . Yii::app()->createAbsoluteUrl('admin/' . Yii::app()->controller->id . '/deleteall') . "';
                document.getElementById('queue-grid-bulk').submit();
        }
});
...
Yii::app()->clientScript->registerScript('ajaxupdate', "
    $('#queue-grid a.ajaxupdate').live('click', function() {
        $.fn.yiiGridView.update('queue-grid', {
            type: 'POST',
            url: $(this).attr('href'),
            success: function() {
...
//            ),
            ...
            array(
                'name' => 'seen',
                'value' => '$data->seen == 1 ? "Yes" : "No"',
                'htmlOptions' => array('style' => 'text-align:right;')
            ),
            array(
                'name' => 'dispense',
                'value' => '$data->dispense == 1 ? "Yes" : "No"',
                'htmlOptions' => array('style' => 'text-align:right;')
            ),
...
//            ),
            array(
                'name' => 'doctor_id',
                'value' => '$data->doctor ? $data->doctor->full_name : ""',
                'htmlOptions' => array('style' => 'text-align:right;')
            ),
//            array(
//                'name' => 'patient_id',
