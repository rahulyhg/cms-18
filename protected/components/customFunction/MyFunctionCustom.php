...
class MyFunctionCustom extends CActiveRecord {
    ...
    /**
     * <Jason>
     * <To delete all child in folder>
     * @param type $source
     */
    public static function deleteAllChild($source, $name = '') {//  "/upload/admin/artist"
    ...
    public static function saveFile($id, $list, $type) {
        //save files
        $valid_formats = array("jpg", "png", "doc", "docx", "xls", "xlsx", "pdf");
        $max_file_size = FileHelper::getMaxFileSize(2); //100 kb
        $imageHelper = new ImageHelper();

        if ($type == 'insurrance_card') {
            //get patient medical insurrance
            $medicalInsurrance = PatientMedicalInsurrance::model()->findByPk($id);
            $imageHelper->createDirectoryByPath("upload/patient/" . $medicalInsurrance->patient_id . '/' . $type);
            $path = Yii::getPathOfAlias("webroot") . '/upload/patient/' . $medicalInsurrance->patient_id . '/' . $type . '/'; // Upload directory
        } else {
            $imageHelper->createDirectoryByPath("upload/patient/" . $id . '/' . $type);
            $path = Yii::getPathOfAlias("webroot") . '/upload/patient/' . $id . '/' . $type . '/'; // Upload directory
        }
        ...
        if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            foreach ($list['name'] as $f => $name) {
                ...
                if ($list['error'][$f] == 0) {
                    ...
                    $matches = preg_match('/^[_a-zA-Z0-9- ]+$/', $filname_without_ext);
                    if (!$matches) {
                        $message[] = "$name is not valid!.";
                        continue; // Skip large files
                    } elseif ($list['size'][$f] > $max_file_size) {
                        ...
                        $filename = $name; //generate filename based on timestamp
                        if (move_uploaded_file($list["tmp_name"][$f], $path . $filename)) {
                            $count++; // Number of successfully uploaded files
                            //Add to db
                            if ($type == 'referral_letter') {
                                $model = new PatientReferralLetter();
                                $model->patient_id = $id;
                            } elseif ($type == 'report') {
                                $model = new PatientReport();
                                $model->patient_id = $id;
                            } else {
                                $model = new PatientInsurranceCard();
                                $model->patient_id = $medicalInsurrance->patient_id;
                                $model->patient_medical_insurrance_id = $id;
                            }
    ...
    /**
     * <Jason>
     * <To generate a number with 6 character>
     * $type = 'D', 'P' ..
     * @return type
     */
    public static function genUniqueCode($type, $length = 6) {
        // $az = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
        $type = Yii::app()->setting->getItem('prefix_reference_no');
        $type = empty($type) ? 'P' : $type;
        $sequence = Patient::model()->nextOrderNumber();
        $repeatLength = $length - strlen($sequence);
        $extStr = str_repeat('0', $repeatLength);
        return $type . $extStr . $sequence;
    }

    public static function buildListPropertyDownloadFE($property_id) {
        $models = Brouchures::getAllBrouchureByProperty($property_id);
        $html = '';
        if (!empty($models)) {
            $html = '<table class="brouchure-fe" style="border: 1px solid; padding: 7px;" cellspacing="0" cellpadding="0" align="center" width="100%">';
            $html .= '<tr>';
            $html .= '<th align="center" width="10%" class="header">No</th>';
            $html .= '<th align="center" width="80%" class="header">File Name</th>';
            $html .= '<th align="center" width="20%" class="header">Download</th>';
            $html .= '</tr>';
            $no = 1;
            foreach ($models as $model) {
                $html .= '<tr>';
                $html .= '<td align="right" width="10%" style="border-right: 1px solid; padding-right: 5px">' . $no . '</td>';
                $html .= '<td align="left" width="80%" style="border-right: 1px solid;">' . $model->file_name . '</td>';
                $html .= '<td align="center" width="20%" style="padding: 5px 15px;"><a style="background:#1e398d; color:#fff; padding: 5px 15px; text-decoration: none;" href="' . $model->generateDownloadLinkFE() . '">Download</a></td>';
                $html .= '</tr>';
                $no ++;
            }
            $html .= '</table>';
    ...
    public static function buildListPropertyDownload($property_id, $email) {
        $models = Brouchures::getAllBrouchureByProperty($property_id);
        $html = '';
        if (!empty($models)) {
            $html = '<table style="border: 1px solid; padding: 7px;" cellspacing="0" cellpadding="0" align="center" width="100%">';
            $html .= '<tr>';
            $html .= '<th align="center" width="80%" style="border-right: 1px solid;">File Name</th>';
            $html .= '<th align="center" width="20%">Download</th>';
            $html .= '</tr>';
            foreach ($models as $model) {
                $html .= '<tr>';
                $html .= '<td align="left" width="80%" style="border-right: 1px solid;">' . $model->file_name . '</td>';
                $html .= '<td align="center" width="20%" style="padding: 5px 15px;"><a style="background:#1e398d; color:#fff; padding: 5px 15px; text-decoration: none;" href="' . $model->generateDownloadLink($email) . '">Download</a></td>';
                $html .= '</tr>';
            }
            $html .= '</table>';
        }
    ...
    public static function stripTagBannerString($string) {
        $string = strip_tags($string);
        return $string;
    }

    public static function forceDownload($model, $fieldName) {
        try {
            if (is_object($model)) {
                $src = $model->uploadFileFolder . "/" . $model->id . "/" . $model->$fieldName;
    ...
    public static function getDispense($queueId)
    {
        $returnData = '';
        $dispenseList = DispenseItems::model()->getDispenseItemByQueue($queueId);
        if (!empty($dispenseList))
        {
            foreach ($dispenseList as $item)
            {
                $returnData .= '- ' . $item->item_name . "<br />";
            }
        }
        return $returnData;
    }
