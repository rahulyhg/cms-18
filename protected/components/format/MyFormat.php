...
//         return str_replace(array('&','<','>','/','\\','"',"'",'?','+'), '', $string);
    ...
    /**
     * @Author: ANH DUNG Oct 01, 2014
     * @Todo: check user can AddBudgetaryCart
     */
    public static function allowAddBudgetaryCart()
    {
        $ok = true;
        if(isset(Yii::app()->user->id) && Yii::app()->user->role_id==ROLE_SELLER){
            $ok = false;
        }
        return $ok;
    }    
    ...
    /**
     * @Author: ANH DUNG Oct 06, 2014
     * @Todo: budgetary cart - gen Customer Ref:
     */
    public static function GenCustomerRef() {
        $uid = Yii::app()->user->id;
        $year = date("y");
        return "R$year-".$uid."-".time();
    }
    /**
     * @Author: ANH DUNG Oct 06, 2014
     * @Todo: budgetary cart - gen Our Ref:
     */
    public static function GenOurRef() {
        $uid = Yii::app()->user->id;
        $year = date("y");
        return "BSQ$year-".$uid."-".time();
    }
