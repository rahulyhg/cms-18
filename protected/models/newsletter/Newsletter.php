...
class Newsletter extends _BaseModel
{
	...
	{
		return array(
			array('newsletter_group_subscriber, subject, content,status, send_time', 'required'),
			array('total_subscriber', 'numerical', 'integerOnly'=>true),
			array('subject', 'length', 'max'=>255),
                        array('send_time', 'date', 'format'=>'yyyy-M-d H:m:s'),
			array('id, subject, send_time, content, created_time, remain_subscribers, total_subscriber, newsletter_group_subscriber,status', 'safe'),
		);
	}
