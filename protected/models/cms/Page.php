...
class Page extends _BasePost
{
	...
	public $defineImageSize = array(
		'featured_image' => array(array('alias' => 'thumb1', 'size' => '369x240')),
	);
	...
	public function getPageTree($publishedOnly = false, $parent = 0, $limitLevel = 0, $notIncluded = 0)
	{
		...
		$criteria->select = 't.title,t.status,t.created_date,t.id,t.parent_id,t.display_order,t.slug,t.level';
	...
	public function getRoot($currentPageID, &$return, $publishedOnly = true)
	{
		...
		{
			$return = $curPage;
			if (count($curPage->id) > 0 && $curPage->parent_id != 0)
			{
				$this->getRoot($curPage->parent->id, $return, $publishedOnly);
			}
			else
			{
				return;
