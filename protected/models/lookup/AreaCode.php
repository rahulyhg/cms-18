...
class AreaCode extends _BaseModel
{
	...
	public static function loadArrNationality()
	{
		$models = self::model()->findAll(array(
			'order' => 'nationality',
			'condition' => 'nationality is not NULL'
		));
		return CHtml::listData($models, 'nationality', 'nationality');
	}

	public static function loadAreaNameOnly()
	{
		$models = self::model()->findAll(array(
			'order' => 'area_name',
		));
		return CHtml::listData($models, 'area_name', 'area_name');
	}

	public static function getAreaCodeSpecific($areaCodeId, $isAddress=false)
	{
		$model = self::model()->findByPk($areaCodeId);
		$country = '';
		if ( isset($model) ) {
			if ( $isAddress ) {
				$country = $model->area_name;
			}
			else{
				$country = $model->area_name . ' (+' . $model->area_code . ')';
			}
		}
		return $country;
	}

	public function getAreaCodeName($areaCodeId)
	{
		$model = self::model()->findByPk($areaCodeId);
