...
<div class="form-type">
    <div class="box-1">
        <div class="title-box clearfix">
            <h2 class="title"><?php echo Yii::t('static', 'Calendar') ?></h2>
...
<input type="hidden" id="ajaxDayEvent" value="<?php echo Yii::app()->createUrl('ajax/GetDoctorAppointment/'); ?>" />
<input type="hidden" id="ajaxWeekEvent" value="<?php echo Yii::app()->createUrl('ajax/GetAppointmentByWeek/'); ?>" />
...
<?php foreach ($blackout as $bo) : ?>
    ...
    switch ($bo->type) {
        case 'holiday': $type = 'Holiday: ';
            break;
        case 'covering':
            $doctor = Doctor::model()->findByPk($bo->doctor_id);
            $coveringDoctor = Doctor::model()->findByPk($bo->covering_doctor_id);
            $type = $coveringDoctor->salutation . ' ' . $coveringDoctor->name . ' Covering ' . $doctor->salutation . ' ' . $doctor->name . ': ';
            break;
        case 'other': $type = 'Others: ';
            break;
    }
    ?>
                {title:"<?php echo $type . $bo->comments ?>",
...
<?php foreach ($events as $doctor_id => $list) : ?>
    ...
    foreach ($list as $event) :
        $eventComment = HString::correctCalendarText($event->comments);
        ?>

        <?php
        if (get_class($event) == 'Appointment'):
            $eventFullComment = HString::correctCalendarText($event->comments);
            $eventComment = HString::excerpt($eventFullComment, 20);
...
<?php endforeach ?>
        ...
        $('#calendar').fullCalendar({
            ...
            dayClick: function(date, jsEvent, view) {
                ...
                if (view.name == 'month' || date.hasTime()) {
                    document.location = '<?php echo Yii::app()->createUrl('appointment/create', array('d' => 'var_d', 't' => 'var_t', 'doctor' => 'var_dt')) ?>'.replace('var_d', curdate).replace('var_t', date.format('HH:mm')).replace('var_dt', doctor_id);
                }
                else {
                    document.location = '<?php echo Yii::app()->createUrl('blackout/create', array('d' => 'var_d', 'doctor' => 'var_dt')) ?>'.replace('var_d', curdate).replace('var_dt', doctor_id);
                }
    ...
    function getMonthEvents(doctor_id) {
        ...
        $.ajax({//Add event for selected date
            ...
            success: function(data) {
                ...
                $.each(data, function() {
                    event = new Object();
                    totalEvent = this.operation + this.fuConsult + this.newConsult + this.chemotherapy + this.others;
    ...
    function getWeekEvents(doctor_id) {
        ...
        urlWeekEvent = $('#ajaxWeekEvent').val();
        $.ajax({//Add event for selected date
            type: "POST",
            dataType: "json",
            async: false,
            ...
            success: function(data) {
                ...
                $.ajax({//Add event for selected date
                    ...
                    cache: false,
                    timeout: 30000,
                    url: urlWeekEvent, //Relative or absolute path to response.php file
        ...
        $.ajax({//Add event for selected date
            ...
            success: function(data) {
                ...
                $.ajax({//Add event for selected date
                    ...
                    cache: false,
                    timeout: 30000,
                    url: urlDateEvent, //Relative or absolute path to response.php file
                    data: {date: current, doctor_id: doctor_id, },
                    success: function(data) {
...
<style type="text/css">
    ...
    .sumary { font-size: 10px !important;}
</style>