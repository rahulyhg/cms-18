    ...
    <tbody>
        ...
        foreach ($model as $m) {
            switch ($m->visit_reason) {
                case 0:
                    $consult++;
                    if (!in_array($m->event_id, $ids)) $consult_not_turn_up++;
                    break;
                case 1:
                    $chemotherapy++;
                    if (!in_array($m->event_id, $ids)) $chemotherapy_not_turn_up++;
                    break;

                default:
                    $others++;
                    if (!in_array($m->event_id, $ids)) $other_not_turn_up++;
                    break;
            }
        ?>
            <tr>
