...
<div class="form-type">
    <div class="box-1">
        <div class="title-box clearfix">
            <h2 class="title"><?php echo Yii::t('static', 'Doctor') ?></h2>
...
foreach ($blackout as $event):
    ...
    $doctor = Doctor::model()->findByPk($event->doctor_id);
    $coveringDoctor = Doctor::model()->findByPk($event->covering_doctor_id);
    $title = $coveringDoctor->salutation . ' ' . $coveringDoctor->name . ' Covering ' . $doctor->salutation . ' ' . $doctor->name . ': ';

    $eventFullComment = HString::correctCalendarText($event->comments);
    $eventComment = HString::excerpt($eventFullComment, 16);
    ?>
                                {title:"<?php echo $title . $eventComment ?>",
                                    ...
                                    resources: ['<?php echo $event->doctor_id; ?>','<?php echo $event->covering_doctor_id; ?>'],
                                    description: "<?php echo $title . $eventFullComment; ?>",
                                    allDay:true,
    ...
    $(document).ready(function() {
        ...
        $('#small-prev').click(function(){
            ...
            if (to_date.getMonth() == curMonth.getMonth()) {
                $('#calendar-doctor').fullCalendar( 'gotoDate', to_date);
            }
            else {
                curMonth.setDate(1);
