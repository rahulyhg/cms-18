...
class AppointmentsController extends BaseAppointment {
    ...
    public function actionIndex() {
        ...
        if (isset($_POST['Appointment'])) {
            ...
            if ($startDate == '')
                ...
                $startDate = HDateTime::formatDate($startDate);

            if ($endDate == '') {
                $endDate = $startDate;
            } else
                $endDate = HDateTime::formatDate($endDate);
    ...
    public function BindPatient($model, $patientId) {
    ...
    public function actionAddAppointment($d = null, $t = null, $doctor = null) {
        ...
        if (isset($_GET['patient_id']) && !empty($_GET['patient_id'])) {
            ...
            $model = $this->BindPatient($model, (int) $_GET['patient_id']);
        }
        $queue = null;
        if (isset($_GET['queue_id']) && !empty($_GET['queue_id'])) {
            ...
            if ($queue) {
                ...
                $model = $this->BindPatient($model, $queue->patient_id);
            }
        }
        ...
        if (isset($_POST['Appointment'])) {
            ...
            if ($model->validate()) {
                ...
                if ($model->save()) {
                    $this->redirect(Yii::app()->createAbsoluteUrl("appointments/index"));
                }
            }
        } else {
            if (!empty($d)) {
    ...
    public function actionDelete($id) {
        ...
        $model = Appointment::model()->findByPk($id);
        if (isset($_POST['Appointment']) && $model) {
            $model->is_deleted = 1;
            $model->update(array('is_deleted'));
            $this->redirect(array('appointments/index'));
        }
    ...
    public function actionRestore($id) {
        ...
        if (isset($_POST['Appointment']) && $model) {
            ...
            $model->is_deleted = 0;
            if (!$model->update(array('is_deleted'))) {
